...
% ── Gauge sector ────────────────────────────────────────────────────
Gauge-sector birefringence ($\mathbf{Q}$ eigenvalues $\{4,4,16\}$): operator structure exact; observability \emph{open}
    ...
    & \emph{Operator structure exact and reproduced by running code:} dcl-core \texttt{exp\_04} extracts $\mathbf{Q}$ from the Peierls plaquette holonomy and matches Paper~I App.~B exactly ($\max|\mathbf{Q}-\mathbf{Q}_{\text{PaperI}}|=0$, eigenvalues $\{4,4,16\}$, axis $(1,1,-1)$) -- no large $N$, no GPU. \emph{Observability OPEN (the reason this row is not \texttt{PASS}):} no symmetry forces isotropy (the $O_h$-average is an external imposition the uniaxial dynamics do not justify); by the \texttt{exp\_01} analogy the anisotropy may survive in observables, where read literally it is a dimension-4, $O(1)$ effect excluded by $\sim$30--37 orders. The sole open item is the \emph{full E+B photon-dispersion order} (electric/magnetic cancellation in the full dispersion -- Paper~I did the magnetic sector only -- is uncomputed); \texttt{exp\_03} (full E+B Peierls dynamics; dcl-core acceptance test~\#5) is decisive and is the genuinely GPU/large-$N$-bound quantity. See \texttt{notes/gauge\_sector\_structural\_conclusion.md}.
    & \texttt{PART} \\
