...
\begin{proof}
    ...
    $V_a\!\times\!V_b$ are the columns of $\operatorname{adj}(M)$, so
...
Two limits are stated plainly rather than hidden. First, the shared dispersion
$\omega^2=k^{\!\top}\varepsilon\,k$ of Eq.~\eqref{eq:shared_dispersion} is
direction-dependent: the common phase speed $v^2(\hat k)$ ranges over the
eigenvalues of $\varepsilon$, i.e.\ a factor of about two between the axis and the
perpendicular plane. This is a \emph{common-mode} speed anisotropy shared by both
polarizations---not birefringence---but it is not small, and its restoration to
isotropy under a full $O_h$ average is a separate question from the polarization
verdict. (Notably, the $O_h$ average that would remove it also renders the
birefringence cancellation trivial; the non-trivial, scale-independent
cancellation proved here lives at the un-averaged operator level, where the speed
anisotropy is also present.) Second, the relative normalization of $\varepsilon$
and $\mu^{-1}$---the tick-weight and one-loop $1/g^2$ factors---is reported, not
fixed; it sets the overall photon speed but, as shown, cannot affect the
birefringence verdict. Accordingly the electric block and its covariant completion
are recorded as complete, while the unconditional birefringence verdict is held
partial pending the isotropy-restoration question and Paper~IV's full
large-$N$ dispersion classification (Table~\ref{tab:audit}).
